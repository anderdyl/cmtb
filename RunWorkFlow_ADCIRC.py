...
#from frontback.frontBackADCIRC import CMSanalyze
from frontback.frontBackADCIRC import ADCIRCsimSetup
from frontback.frontBackUn import CMSsimSetup




def Master_Unstruct_run(inputDict):
    """This function will run CMS, WW3, or ADCIRC with any version prefix given a start time
    designed to be for unstructured work flow

    Args:
      inputDict: a dictionary that is read from the input yaml
        model(str): available (ww3, cms, adcirc)
    Returns:
      None
    ...
    generateFlag = inputDict.get('generateFlag', False)
    runFlag = inputDict.get('runFlag', False)
    analyzeFlag = inputDict.get('analyzeFlag', False)
    pFlag = inputDict.get('pFlag', False)
        ...
        try:
            ...
            if generateFlag == True:
                if model == 'cms':
                    ...
                    temp = ADCIRCsimSetup(time, inputDict=inputDict)
            ...
            #     dt = DT.datetime.now()
            #
            #     simOutput = check_output(
            #         codeDir + '%s %s.sim' % (inputDict['modelExecutable'], ''.join(time.split(':'))), shell=True)
            #
...
# print(
    #     '------------------------------------\n************************************\n------------------------------------\n')
    # print('Master workflow for {} simulation'.format(model))
    # print('Simulation for Start: %s     Finish: %s ' % (projectStart, projectEnd))
    # print('The simulation is run for "%s" version in "%s" mode' % (version_prefix, modelMode))
    # print(
    #     '------------------------------------\n************************************\n------------------------------------\n')
    # # ________________________________________________ RUN LOOP ________________________________________________
    # if generateFlag == True:
    #     print('------ getting external model input parameters {}: ------\n'.format(DT.datetime.now()))
    #     ADCIRCsimSetup(startTime=startTime, inputDict=inputDict)
    #
...
if __name__ == "__main__":
    ...
    print('USACE FRF Coastal Model Test Bed : ADCIRC')
    ...
    try:
        ...
        with open(os.path.join(yamlLoc), 'r') as f:
            inputDict = yaml.load(f, Loader=yaml.FullLoader)
    except:
        raise IOError(
            'Input YAML file required.  See yaml_files/TestBedExampleInputs/CMS_Input_example for example yaml file.')

    temp = Master_Unstruct_run(inputDict=inputDict)
